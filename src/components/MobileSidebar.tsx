...
export function MobileSidebar({ userEmail, fullName }: MobileSidebarProps) {
	...
	const navigation = [
		...
		{
			name: "Maintenance",
			href: getDashboardURL("maintenance"),
			icon: WrenchIcon,
		},
		{
			name: "Utilities",
			href: getDashboardURL("utilities"),
			icon: IconWavesElectricity,
		},
		{
			name: "Documents",
			href: getDashboardURL("documents"),
			icon: IconSignature,
		},
		{
			name: "Settings",
		...
		{
			name: "Delete",
			href: getDashboardURL("delete-property"),
	...
	return (
		<>
			...
			<div
				className={`fixed inset-0 flex lg:hidden z-40 ${sidebarOpen ? "fixed w-full h-lvh translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out bg-gray-50`}
			>
				<div className="flex grow flex-col overflow-y-auto px-4 mt-16">
					<nav className="flex flex-1 flex-col ">
						<ul
							role="list"
							className="flex flex-1 flex-col divide-y divide-gray-200"
						>
							{navigation.map((item) => (
								<li key={item.name}>
									<Link
