...
export function PropertiesGrid() {
	...
	const fuse = new Fuse(properties || [], {
		keys: ["street_address", "city", "state"],
		threshold: 0.4,
	});
				...
				<div className="flex items-center justify-end w-full sm:w-auto">
					<Dropdown>
						<DropdownButton outline className="mr-2.5 h-10 bg-gray-50">
							<span className="text-sm block">Sort by</span>
							<ChevronDownIcon />
						</DropdownButton>
						<DropdownMenu className="min-w-32">
							<DropdownItem onClick={() => setSortBy("name")}>
								<div
									className={`text-sm ${sortBy === "name" && "font-semibold"}`}
								>
									Address
								</div>
							</DropdownItem>
							<DropdownItem onClick={() => setSortBy("city")}>
								<div
									className={`text-sm ${sortBy === "city" && "font-semibold"}`}
								>
									City
								</div>
							</DropdownItem>
							<DropdownItem onClick={() => setSortBy("state")}>
								<div
									className={`text-sm ${sortBy === "state" && "font-semibold"}`}
								>
									State
								</div>
							</DropdownItem>
					...
					<Button
						disabled={addPropertyClicked}
						className="w-40 h-10 cursor-default"
						onClick={handleClick}
					>
						<div className="">
							{addPropertyClicked ? (
					...
					{isLoading ? (
						Array.from({ length: 3 }).map((_, index) => (
							<div
								...
								key={index}
								className="rounded-lg p-4 h-28 lg:h-36 shadow-sm bg-gray-50 ring-1 ring-gray-300 w-full max-w-md"
							>
							...
							>
								<div className="absolute top-3 right-3">
									<Dropdown>
										<DropdownButton
											plain
											className="h-7 w-6 p-0 bg-transparent transition ease-in-out duration-200"
										>
										...
										<DropdownMenu>
											...
											<DropdownItem
												href={`/dashboard/${property.id}/delete-property`}
											>
												<div className="flex items-center">
									...
									<div className="absolute bottom-4 left-4 flex items-center gap-x-1">
										{property?.tenants && property.tenants.length > 0 ? (
											<>
												...
												<span className="text-gray-700 tracking-tight text-sm">
													{property.tenants.length}{" "}
													{property.tenants.length === 1 ? "tenant" : "tenants"}
												</span>
											</>
										) : (
											<>
